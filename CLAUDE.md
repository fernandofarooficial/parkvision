...
Filtro adicional (via JOIN/NOT EXISTS com `movcar` e `deparaplacas`): só entram fotos de eventos com `movcar.contav = 0` (ainda pendentes, aguardando confirmação do operador — ver semântica de `contav` na seção Banco de Dados), `movcar.placa != '*ERROR*'` (sentinel usado em `dblib`/`vplib`/`operlib` para placa não reconhecida pelo OCR) e cuja placa lida (`logbruto.placalida`) **não** esteja em `deparaplacas.placade` (`placade CHAR(7)` PK, `placapara` = placa correta correspondente) — se já existe correção conhecida para aquela placa, o operador não precisa mais olhar a foto. O filtro usa `placalida` e não `movcar.placa` porque `movcar.placa` já vem corrigido por `vplib.process_heimdall_plate` (é o `placapara`, não o `placade`). Ou seja, a tela De<>Para mostra apenas capturas com placa lida corretamente, ainda não processadas/confirmadas e sem correção já mapeada — não é mais só "as últimas fotos do condomínio" em bruto.
...
Cada card também tem os botões **"Placa = Anterior"** e **"Placa = Posterior"** (desabilitados quando o respectivo movimento vizinho não existe), que gravam manualmente o mapeamento de correção: `placade` = placa lida do card (`placalida`), `placapara` = placa do movimento vizinho escolhido pelo operador. `POST /api/deparaplacas/criar` → `vplib.criar_mapeamento_deparaplacas()` (valida formato via `validar_formato_placa`, rejeita `placade == placapara`, `INSERT ... ON DUPLICATE KEY UPDATE` em `deparaplacas`). Não recarrega a lista de fotos automaticamente — o card fica marcado com "Mapeado para X" e os botões desabilitados; a foto some da lista só na próxima vez que o modal for reaberto (filtro de `deparaplacas.placade` da query em `obter_ultimas_fotos`).

**Botão Consultar:** abre `#modalEscolherPlacaConsulta` (input de placa) → `#modalConsultaVeiculo` (`templates/base.html`, compartilhado entre todas as telas que estendem `base.html`, não só `veiculos.html`). Chama `abrirConsultaVeiculo(placa)` → `GET /api/consulta-veiculo/<placa>` → `listlib.consulta_veiculo()`, que devolve dados cadastrais (`cadveiculo` + marca/modelo/cor) e todas as permissões da placa em qualquer condomínio (`cadperm` com status calculado em Python: `VIGENTE`/`INDEFINIDA`/`FUTURA`/`VENCIDA`). O mesmo modal é reaproveitado pelo botão "Consultar Veículo" dentro do modal de Detalhes do Veículo (`onclick="abrirConsultaVeiculo(...)"` em `templates/veiculos.html`).

A resposta também traz `placas_de`: todas as `deparaplacas.placade` cujo `placapara` seja a placa consultada — ou seja, todas as placas mal lidas pelo OCR que já foram mapeadas manualmente (via botões "Placa = Anterior/Posterior" do De<>Para) para essa placa. Renderizadas como badges na seção "Placas lidas mapeadas para esta (De<>Para)" (`#cv-depara`), oculta quando a lista vem vazia. Diferente dos outros campos do modal, `placas_de` é calculado e retornado mesmo quando a placa não está cadastrada em `cadveiculo` (`success: false`) — o mapeamento em `deparaplacas` pode existir independente de cadastro.
