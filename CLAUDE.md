...
Cada card exibe tanto a placa lida (`placalida`, o que a câmera/OCR capturou) quanto a placa de trabalho (`placa_trabalho` = `movcar.placa`, já corrigida por `vplib.process_heimdall_plate`) — ajuda o operador a perceber de cara se o sistema já está usando uma placa diferente da que aparece na foto.
